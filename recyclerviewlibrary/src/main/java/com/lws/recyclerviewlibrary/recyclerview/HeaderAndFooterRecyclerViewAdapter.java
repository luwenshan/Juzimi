...
/**
 * Created by Wenshan.Lu on 2016/11/25.
 * <p>
 * RecyclerView.Adapter with Header and Footer View.
 */
...
public class HeaderAndFooterRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void setAdapter(RecyclerView.Adapter<RecyclerView.ViewHolder> adapter) {
        //如果已经设置过adapter，则重新设置后，其对应的Item会被删除，但HeaderViews不会被删除
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        int headerViewsCountCount = getHeaderViewsCount();
        //每一个HeaderView和FooterView都是一种viewType
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (position >= headerViewsCountCount && position < headerViewsCountCount + mInnerAdapter.getItemCount()) {
            mInnerAdapter.bindViewHolder(holder, position - headerViewsCountCount);
        } else {
    ...
    /**
     * 每一个HeaderView 和 FooterView 都单独对应一个ItemViewType
     * TYPE_HEADER_VIEW = Integer.MIN_VALUE;
     * TYPE_FOOTER_VIEW = Integer.MIN_VALUE + 1;
     * [Integer.MIN_VALUE, Integer.MIN_VALUE + getHeaderViewsCount()) 对应HeaderView 的 ItemViewType
     * [Integer.MIN_VALUE + 1, Integer.MIN_VALUE + 1 + getFooterViewsCount()) 对应FooterView 的 ItemViewType
     *
     * @param position
     * @return
     */
    ...
    public int getItemViewType(int position) {
        ...
        int headerViewCountCount = getHeaderViewsCount();

        if (position < headerViewCountCount) {
            return TYPE_HEADER_VIEW + position;
        } else if (headerViewCountCount <= position && position < headerViewCountCount + innerCount) {
            int innerItemViewType = mInnerAdapter.getItemViewType(position - headerViewCountCount);
            if (innerItemViewType >= Integer.MAX_VALUE / 2) {
                throw new IllegalArgumentException("your adapter's return value of getViewTypeCount() must <Integer.MAX_VALUE/2");
            }
            return innerItemViewType + Integer.MAX_VALUE / 2;
        } else {
            return TYPE_FOOTER_VIEW + position - headerViewCountCount - innerCount;
        }
    }
